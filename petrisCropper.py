#!/user/bin/python3

import numpy as np
import argparse
import cv2
import os
from skimage import measure
from imutils import contours
import imutils

# construct the argument parser and parse the arguments
ap = argparse.ArgumentParser()
ap.add_argument("-c", "--circlesize", required = True, help = "Size of circle", type=int)
args = vars(ap.parse_args())

wd = os.getcwd()

if 'mnt' in wd:
	pictureDir = "/mnt/c/Users/hmmar/Google Drev (hannahmariemartiny@gmail.com)/iGEM 2018/Wet lab/Pictures of plates/"
else:
	pictureDir = "C:\\Users\\hmmar\\Google Drev (hannahmariemartiny@gmail.com)\\iGEM 2018\\Wet lab\\Pictures of plates\\"

dateDirs = os.listdir(pictureDir)
dateDirs = [x for x in dateDirs if x.endswith("plates")] # only work with correct picture folders

for dateDir in dateDirs:
	subDestDir = wd + '/cropped_pic/' + dateDir + '/'

	# print("Date:", )

	if not os.path.exists(subDestDir):
	    os.makedirs(subDestDir)

	destDir = subDestDir

	pictureFiles = os.listdir(pictureDir + dateDir)

	pictureFiles = [x for x in pictureFiles if x.endswith('.jpg')]

	for i, pictureFile in enumerate(pictureFiles):
		print("{0}/{1}".format(i+1, len(pictureFile)), end = '\r')
	# CODE BELOW TAKEN FROM
	# https://www.pyimagesearch.com/2014/07/21/detecting-circles-images-using-opencv-hough-circles/
	#  https://stackoverflow.com/questions/36911877/cropping-circle-from-image-using-opencv-python?rq=1

		# load image
		image1 = cv2.imread(pictureDir + dateDir +'/' + pictureFile)
		image = cv2.imread(pictureDir + dateDir +'/' + pictureFile, 0)

		img1 = image1.copy()
		img = image.copy()
		gray = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
		ret, thresh = cv2.threshold(gray, 50, 255, cv2.THRESH_BINARY)

		# Create mask
		height,width = img.shape
		mask = np.zeros((height,width), np.uint8)

		edges = cv2.Canny(thresh, 100, 200)
		#cv2.imshow('detected ',gray)
		cimg=cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
		circles = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, 1, 10000, param1 = 50, param2 = 30, minRadius = 0, maxRadius = 0)
		for i in circles[0,:]:
		    i[2]=i[2]+4
		    # Draw on mask
		    cv2.circle(mask,(i[0],i[1]),i[2],(255,255,255),thickness=-1)

		# Copy that image using that mask
		masked_data = cv2.bitwise_and(img1, img1, mask=mask)

		# Apply Threshold
		_,thresh = cv2.threshold(mask,1,255,cv2.THRESH_BINARY)

		# Find Contour
		contours = cv2.findContours(thresh,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
		x,y,w,h = cv2.boundingRect(contours[0])

		# Crop masked_data
		crop = masked_data[y:y+h,x:x+w]

		# print("Created cropped image")
		#Code to close Window
		# cv2.imshow('detected Edge',img1)
		# cv2.imshow('Cropped Eye',crop)
		# cv2.waitKey(0)

		# create copy
		output_crop = crop.copy()

		# convert color to gray scale
		gray_crop = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)

		# detect circles in the image
		circles = cv2.HoughCircles(gray_crop, cv2.HOUGH_GRADIENT, 1.2, args["circlesize"])#, minRadius=0,maxRadius=5)

		# ensure at least some circles were found
		if circles is not None:
			# convert the (x, y) coordinates and radius of the circles to integers
			circles = np.round(circles[0, :]).astype("int")

			# loop over the (x, y) coordinates and radius of the circles
			for (x, y, r) in circles:
				# draw the circle in the output image, then draw a rectangle
				# corresponding to the center of the circle
				#cv2.circle(output_crop, (x, y), r, (0, 255, 0), 4)
				cv2.rectangle(output_crop, (x - 5, y - 5), (x + 5, y + 5), (0, 128, 255), -1)

			# show the output image
			# cv2.imshow("output", np.hstack([crop, output_crop]))
			# cv2.waitKey(0)
			filename = destDir + pictureFile
			cv2.imwrite(filename, output_crop)


		cv2.destroyAllWindows()
